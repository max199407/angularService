...
function execute(cfg, symbols, initialState, shouldTrackSymbol = () => true) {
    ...
    function isForInOfLoop(block) {
        return navigation_1.is(block.loopingStatement, ts.SyntaxKind.ForInStatement, ts.SyntaxKind.ForOfStatement);
    }
    function isStatement(block) {
        return navigation_1.is(block.branchingElement, ...navigation_1.CONDITIONAL_STATEMENTS, ...navigation_1.LOOP_STATEMENTS);
    }
    function isSwitch(block) {
        return navigation_1.is(block.branchingElement, ts.SyntaxKind.SwitchStatement);
    }
}
